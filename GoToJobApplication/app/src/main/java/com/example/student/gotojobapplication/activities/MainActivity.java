...
public class MainActivity extends AppCompatActivity {
    ...
    private void getUsersTwo() {
        final Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(getString(R.string.base))
                .addConverterFactory(GsonConverterFactory.create())
                .build();
        final APIServices service = retrofit.create(APIServices.class);
        service.randomUsers(6).enqueue(new Callback<ModelForRecycler>() {
            @Override
            public void onResponse(Call<ModelForRecycler> call, Response<ModelForRecycler> response) {
                if (response != null && response.isSuccess()) {
                    ...
                    Toast.makeText(MainActivity.this, R.string.problem_with_response, Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<ModelForRecycler> call, Throwable t) {
                Toast.makeText(MainActivity.this, R.string.try_load_again, Toast.LENGTH_SHORT).show();
            }
        });
    }
